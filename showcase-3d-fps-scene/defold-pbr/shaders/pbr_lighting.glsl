...
#define PBR_LIGHTING
	...
	{
		const float depth_bias = 0.0002;
		// const float depth_bias = 0.00002; // for perspective camera

		float shadow = 0.0;

		ivec2 texture_size = textureSize(tex_occlusion, 0);
		...
		for (int x = -1; x <= 1; ++x)
		{
			for (int y = -1; y <= 1; ++y)
			{
				float depth = texture2D(tex_occlusion, depth_data.st + vec2(float(x),float(y)) * texel_size).r;
	...
	vec3 applyOcclusion(PBRParams params, LightingInfo lightInfo, vec3 colorIn)
	{
		...
		lightInfo.occlusion *= 1.0 - shadow_calculation(depth_proj.xyzw);
